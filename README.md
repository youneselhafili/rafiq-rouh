...
- تخزين Local-first دائم مع مزامنة Firestore اختيارية: انقطاع Firebase أو امتلاء الحصة لا يوقف البوت ولا يمنع الحفظ.
...
- Firebase اختياري كنسخة بعيدة ومزامنة بين عمليات التشغيل؛ التخزين المحلي يعمل بدونه.
...
# اختياري: إذا غاب أو تعذر Firebase يستمر البوت محلياً
...
يحفظ البوت كل تغيير محلياً أولاً، ثم يزامنه مع Firestore إذا كانت متاحة. توجد مرايا التخزين داخل:

- `data/guilds/` لإعدادات السيرفر والرتب واللوحات وحالة المجدولات.
- `data/users/` لإعدادات وتنبيهات الرسائل الخاصة.
- `data/personal-khatmas/` لتقدم الورد الشخصي لكل عضو.

إذا فشلت Firestore بسبب انقطاع، quota أو خطأ مؤقت، تبقى الكتابة المحلية ناجحة وتُعلّم كمزامنة معلقة. عند رجوع Firestore يحاول البوت رفع النسخة المحلية قبل قبول نسخة بعيدة أقدم. يجب الاحتفاظ بمجلد `data/` على قرص دائم وأخذ نسخة احتياطية منه. عند تشغيل البوت على VPS واحد يحمي القفل المحلي من نسختين؛ تشغيله على عدة خوادم أثناء انقطاع Firestore قد يسبب تكرار المجدولات.
...
## الصيانة اليومية على VPS

يمكن تشغيل `scripts/vps-maintenance.sh` مرة كل 24 ساعة عبر cron. ينظف الملفات
المؤقتة القديمة الخاصة بمستخدم البوت، ويمنع تضخم سجلات PM2، ويعيد تشغيل البوت،
ثم يفحص Dashboard API. لا يحذف `data/` أو `.env` أو النسخ الاحتياطية.
...
## سجلات الـ Blacklist (الروابط المعطلة)

يتتبع نظام السجلات تلقائياً كل حدث يتعلق بالروابط الصوتية المعطلة عبر حدثين مستقلين يُرسَلان إلى قناة السجلات:

| الحدث | المستوى | متى يُطلق | ما يحمله |
|---|---|---|---|
| `Audio link blacklisted` | ❌ خطأ | عند إضافة رابط صوتي إلى القائمة السوداء بعد فشله | اسم القارئ، رقم السورة، سبب الفشل، عدد محاولات الاسترداد |
| `Audio link error` | ⚠️ تنبيه | عند حدوث خطأ مؤقت في تشغيل رابط (دون حظر) | تفاصيل الخطأ والرابط المتأثر |

### زر الإدارة المباشرة

عندما يصل البوت لقناة السجلات برسالة `Audio link blacklisted` أو `Audio link error`، يُرفق معها تلقائياً زر **📋 إدارة الروابط المعطلة** يفتح لوحة تحكم خاصة تعرض:

- قائمة الروابط المعطلة مع سبب الحظر وتاريخ الإضافة.
- عدد محاولات الاسترداد والموعد المتوقع للفحص التالي (كل 3 أيام).
- قائمة منسدلة لاختيار أي رابط وحذفه يدوياً لإعادة تشغيله فوراً.
- زر تحديث القائمة في الوقت الفعلي.

### آلية الاسترداد التلقائي

الروابط المحظورة لا تُحذف نهائياً؛ يعيد البوت فحصها تلقائياً كل **3 أيام**. إذا نجح الرابط عند إعادة الفحص يُزال من القائمة السوداء ويعود للتشغيل العادي. يمكن دائماً تجاوز انتظار 3 أيام عبر لوحة الإدارة.
