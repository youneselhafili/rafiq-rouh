...
- لوحة ورد شخصي ثابتة داخل السيرفر: إعداد مستقل لكل عضو، تذكير يومي بالمنشن، وصفحات Ephemeral لا يراها غير القارئ، مع حفظ التقدم بعد كل صفحة واستدراك أيام الانقطاع.
- معاينة صفحة القرآن من لوحة `/nakhtim` قبل الحفظ.
- أذكار عشوائية مع فصل القنوات: أذكار الأذان والوضوء في قناة الأذان، والاستيقاظ والنوم وباقي الأنواع في قناة الأذكار العامة، مع الصلاة على النبي ﷺ وتذكير الجمعة وسورة الكهف.
...
| `/setup_roles` | Manage Server | ربط رتب المنشن بالأذان والأذكار والجمعة والصلوات والختمة |
...
استعمل `/nakhtim`، اختر نوع الختمة والقناة أو DM، ثم فعّلها واحفظ. تُرسل أول دفعة فور نجاح الحفظ، ثم تستمر الدفعات يومياً في 08:00 بتوقيت مكة. الحماية اليومية تمنع تكرار دفعتين إذا تم الحفظ قريباً من الموعد. يمكن اختيار رتبة منشن الختمة من `/setup_roles`؛ وإذا لم تُحدد رتبة، تُرسل الصفحات بدون منشن ولا يستعمل النظام `@everyone`. زر **معاينة صفحة القرآن** يعرض الصفحة الحالية بدون تغيير التقدم. في الوضع الرمضاني يمكن تحديد عدد الختمات.

لنشر الورد الشخصي، اختر **قناة الورد الشخصي** المستقلة من لوحة `/nakhtim` أو أدخل معرّفها من زر **إدخال ID القنوات**، ثم اضغط **نشر لوحة الورد الشخصي**. اختيارها لا يغيّر **قناة الختمة الجماعية** التي تستقبل صفحات الختمة اليومية. يضبط كل عضو خطته بنفسه، ويضغط **استلم ورد اليوم** ليقرأ صفحةً بعد صفحة في رد Ephemeral. يحفظ البوت الصفحة فور تأكيد قراءتها، ويجمع الصفحات غير المقروءة دون تجاوزها، ويسمح بالقراءة الإضافية. عند الصفحة 604 يعرض دعاء الختم والإحصاءات، ثم يتيح إعادة الختمة بالإعدادات نفسها أو تغييرها. يستعمل التذكير اليومي رتبة الختمة المضبوطة في `/setup_roles` إن وُجدت.
